...
function RefreshData($Timer) {
	...
	if(Test-Path $global:tf) {
		...
		$c_srv=0
		$c_srv_all=$global:SrvFile.Count
		for($i=0;$i -lt $tabs.Count;$i++) {
			$global:SrvFile | Where-Object { $_.Grp -eq $tabs[$i].Name } | Foreach-Object {
				$c_srv++
				$srv=$_.Name
				Write-Progress -Activity "Requesting sessions from servers." -Status "$c_srv of $c_srv_all requested." -CurrentOperation $srv -PercentComplete ($c_srv/$c_srv_all*100)
				$f="$global:tf\$srv.txt"
				set-content $f ""
				$shadow=$false
				if($_.Shadow -eq 1) {
		...
		Write-Progress -Activity "Requesting sessions from servers." -Status "Completed." -Completed
		$T=Get-Date
		$to=0
		while((Get-Job | Where-Object { $_.Name -like "qwinsta_*" -and $_.State -eq "Running"}).Count -gt 0) {
			$to=((Get-Date) - $T).TotalSeconds
			$c_left=(Get-Job | Where-Object { $_.Name -like "qwinsta_*" -and $_.State -eq "Running"}).Count
			$c_all=(Get-Job | Where-Object { $_.Name -like "qwinsta_*"}).Count
			$add="";
			Get-Job | Where-Object { $_.Name -like "qwinsta_*" -and $_.State -eq "Running"} | Sort-Object { $_.Name} | foreach-object {
				if($add -ne "") {
					$add="$add, "
				}
				$add="$add$($_.Name.Replace(`"qwinsta_`",`"`"))"
			}
			Write-Progress -Activity "Waiting for background jobs to complete." -Status "$(($c_all-$c_left)) of $c_all finished ($to of max $global:Timeout seconds)." -CurrentOperation $add -PercentComplete (($c_all-$c_left)/$c_all*100)
			if($to -gt $global:Timeout) {
				$srv_timeout=@()
				Get-Job | Where-Object { $_.Name -like "qwinsta_*" -and $_.State -eq "Running"} | Sort-Object { $_.Name} | foreach-object {
					$srv_timeout+=$_.Name.Replace("qwinsta_","")
				}
				"The following servers timed out:"
				$srv_timeout
				break
			}
		}
		Write-Progress -Activity "Waiting for background jobs to complete." -Status "All finished." -Completed
		Get-Job | Where-Object { $_.Name -like "qwinsta_*" } | Remove-Job -Confirm:$false
		$c_srv=0
		$global:TempSrv | Foreach-Object {
			$c_srv++
			$srv=$_.Name
			$f="$global:tf\$srv.txt"
			Write-Progress -Activity "Reading sessions of servers." -Status "$c_srv of $c_srv_all" -CurrentOperation $srv -PercentComplete ($c_srv/$c_srv_all*100)
			if(Test-Path $f) {
				AddSessions $f $srv
				del $f 2>$null
				if(Test-Path $f) {
		...
		Write-Progress -Activity "Reading sessions of servers." -Status "Completed" -Completed
